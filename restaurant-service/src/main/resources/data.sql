...
INSERT INTO menu_items (restaurant_id, name, price, description, category, available, created_at, updated_at)
SELECT 1, 'Burger Bò Pho Mát Đặc Biệt', 85000.00, 'Thịt bò Úc, phô mai Cheddar, sốt BBQ', 'Burger', TRUE, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM menu_items WHERE restaurant_id = 1 AND name = 'Burger Bò Pho Mát Đặc Biệt');

INSERT INTO menu_items (restaurant_id, name, price, description, category, available, created_at, updated_at)
SELECT 1, 'Burger Gà Giòn Cay', 75000.00, 'Thịt gà giòn, sốt cay đặc biệt', 'Burger', TRUE, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM menu_items WHERE restaurant_id = 1 AND name = 'Burger Gà Giòn Cay');

INSERT INTO menu_items (restaurant_id, name, price, description, category, available, created_at, updated_at)
SELECT 1, 'Khoai Tây Phô Mai Tỏi', 55000.00, 'Khoai tây chiên, sốt phô mai và tỏi', 'Side Dish', TRUE, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM menu_items WHERE restaurant_id = 1 AND name = 'Khoai Tây Phô Mai Tỏi');

INSERT INTO menu_items (restaurant_id, name, price, description, category, available, created_at, updated_at)
SELECT 1, 'Combo Gia Đình (3 Burger + Nước)', 250000.00, 'Combo tiết kiệm cho 3 người', 'Combo', TRUE, NOW(), NOW()
...
INSERT INTO menu_items (restaurant_id, name, price, description, category, available, created_at, updated_at)
SELECT 2, 'Set Sushi Cá Hồi (8 miếng)', 180000.00, 'Cá hồi tươi, cơm cuộn', 'Sushi', TRUE, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM menu_items WHERE restaurant_id = 2 AND name = 'Set Sushi Cá Hồi (8 miếng)');

INSERT INTO menu_items (restaurant_id, name, price, description, category, available, created_at, updated_at)
SELECT 2, 'Tempura Tôm Lớn', 120000.00, 'Tôm sú tẩm bột chiên giòn', 'Appetizer', TRUE, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM menu_items WHERE restaurant_id = 2 AND name = 'Tempura Tôm Lớn');

INSERT INTO menu_items (restaurant_id, name, price, description, category, available, created_at, updated_at)
SELECT 2, 'Cơm Cuộn California', 95000.00, 'Cua, bơ, dưa chuột và mè', 'Maki', TRUE, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM menu_items WHERE restaurant_id = 2 AND name = 'Cơm Cuộn California');

INSERT INTO menu_items (restaurant_id, name, price, description, category, available, created_at, updated_at)
SELECT 2, 'Súp Miso Truyền Thống', 35000.00, 'Súp đậu tương, rong biển và đậu phụ', 'Soup', TRUE, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM menu_items WHERE restaurant_id = 2 AND name = 'Súp Miso Truyền Thống');

INSERT INTO menu_items (restaurant_id, name, price, description, category, available, created_at, updated_at)
SELECT 2, 'Salad Rong Biển', 60000.00, 'Rong biển Wakame trộn giấm', 'Salad', TRUE, NOW(), NOW()
...
INSERT INTO menu_items (restaurant_id, name, price, description, category, available, created_at, updated_at)
SELECT 3, 'Chè Ba Màu Thập Cẩm', 30000.00, 'Đậu xanh, đậu đỏ, sương sáo, nước cốt dừa', 'Chè', TRUE, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM menu_items WHERE restaurant_id = 3 AND name = 'Chè Ba Màu Thập Cẩm');

INSERT INTO menu_items (restaurant_id, name, price, description, category, available, created_at, updated_at)
SELECT 3, 'Tào Phớ Hạt Sen', 35000.00, 'Tào phớ mềm với hạt sen và nước đường gừng', 'Tráng Miệng', TRUE, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM menu_items WHERE restaurant_id = 3 AND name = 'Tào Phớ Hạt Sen');

INSERT INTO menu_items (restaurant_id, name, price, description, category, available, created_at, updated_at)
SELECT 3, 'Sữa Chua Nếp Cẩm Lạnh', 35000.00, 'Sữa chua dẻo ăn kèm nếp cẩm lên men', 'Tráng Miệng', TRUE, NOW(), NOW()
...
INSERT INTO menu_items (restaurant_id, name, price, description, category, available, created_at, updated_at)
SELECT 4, 'Spaghetti Carbonara Nguyên Bản', 130000.00, 'Mì Ý sốt kem trứng và thịt heo muối (guanciale)', 'Pasta', TRUE, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM menu_items WHERE restaurant_id = 4 AND name = 'Spaghetti Carbonara Nguyên Bản');

INSERT INTO menu_items (restaurant_id, name, price, description, category, available, created_at, updated_at)
SELECT 4, 'Pizza Bốn Mùa (Size L)', 250000.00, 'Bốn loại topping khác nhau trên một chiếc pizza', 'Pizza', TRUE, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM menu_items WHERE restaurant_id = 4 AND name = 'Pizza Bốn Mùa (Size L)');

INSERT INTO menu_items (restaurant_id, name, price, description, category, available, created_at, updated_at)
SELECT 4, 'Lasagna Bò Phô Mai', 145000.00, 'Mì lá xếp lớp với sốt bò băm và phô mai', 'Pasta', TRUE, NOW(), NOW()
...
INSERT INTO menu_items (restaurant_id, name, price, description, category, available, created_at, updated_at)
SELECT 5, 'Phở Bò Đặc Biệt Tái Nạm Gầu', 80000.00, 'Phở bò tái, nạm, gầu, nước dùng gia truyền', 'Main Course', TRUE, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM menu_items WHERE restaurant_id = 5 AND name = 'Phở Bò Đặc Biệt Tái Nạm Gầu');

INSERT INTO menu_items (restaurant_id, name, price, description, category, available, created_at, updated_at)
SELECT 5, 'Phở Gà Lá Chanh', 65000.00, 'Phở gà xé, da giòn, thơm mùi lá chanh', 'Main Course', TRUE, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM menu_items WHERE restaurant_id = 5 AND name = 'Phở Gà Lá Chanh');

INSERT INTO menu_items (restaurant_id, name, price, description, category, available, created_at, updated_at)
SELECT 5, 'Bánh Phở Trộn', 70000.00, 'Phở không nước dùng, trộn với sốt và thịt', 'Main Course', TRUE, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM menu_items WHERE restaurant_id = 5 AND name = 'Bánh Phở Trộn');

INSERT INTO menu_items (restaurant_id, name, price, description, category, available, created_at, updated_at)
SELECT 5, 'Quẩy Giòn', 10000.00, 'Quẩy chiên giòn ăn kèm phở', 'Side Dish', TRUE, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM menu_items WHERE restaurant_id = 5 AND name = 'Quẩy Giòn');

INSERT INTO menu_items (restaurant_id, name, price, description, category, available, created_at, updated_at)
SELECT 5, 'Trà Đá', 5000.00, 'Trà đá giải khát', 'Drink', TRUE, NOW(), NOW()
